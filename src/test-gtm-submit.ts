...
async function main() {
  ...
  const name = `Submit Test ${stamp}`;
  const notes = 'Automated submit test';
  ...
  console.log('Submit test completed successfully.');
}

main().catch((err) => {
  console.error('GTM Submit test failed:', err.message || err);
  process.exit(1);
});
